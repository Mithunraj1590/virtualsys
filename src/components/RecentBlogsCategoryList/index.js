...
import Style from "./RecentBlogsCategoryList.module.scss";
import Link from "next/link";
import { getPageContent } from "../../../lib/pages";
import { Nav } from "react-bootstrap";
import RecentBlogBlocks from "../RecentBlogBlocks";
import Icon from "../Layout/Icons";

const RecentBlogsCategoryList = ({ props }) => {
  ...
  const selectedTopic = async (value,page) => {
    ...
    const data = await getPageContent(`blog/filter?offset=${page}&limit=3&category=${value}`);
  ...
  const getDataLoadMore = async (value, page) => {
    ...
    const data = await getPageContent(
      `blog/filter?offset=${page}&limit=3&category=${value}`
    );
    setloading(false);
            ...
            <>
              {result?.length > 0 ? (
                ...
                <div className={Style.recent_news_wrapper}>
                  {result?.map((value, t) => {
                     return <RecentBlogBlocks props={value} key={t} />;
                  })}
                  </div>

                  {count > result?.length && (
                    <button className={`d-block m-auto ${Style.loadmorebtn}`}
                    ...
                    >
                      ...
                      <div
                          className={`arrow_text text-center ${Style.arrow_text}`}
                      >
...
export default RecentBlogsCategoryList;
